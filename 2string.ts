var str:string="Hello How are you";
var str2:string='Hello How are you';
var str3:string=`Hello How are you`;

var age :number=30;
var userName:string| null="Anil sidhu";

//no. ko string mai convert kiya
var info:string=`my name is ${userName} and age is ${age} years`
console.log(info);

var num:number=100;
// var data:string = num.toString();-->string mai convert kiya
// var data:string = " "+ num;
var boolData=true;//bolean ko kiya h
// var data:string = boolData.toString();
// var data:string = "" +boolData;


// var item:boolean=true;
// var other:boolean;
// other=item
// console.log(other);

var str10="hey this is anil"